...
## Eliminating `any` Types in CMS / Generated Code

When working with generated schema types from a CMS (like Sanity TypeGen) or complex API responses, **never fall back to `any`**. Instead, use TypeScript utilities to extract the exact strict types you need from the generated root types.

### Extracting from Union Arrays
When a generated query result (e.g., `PAGE_QUERY_RESULT`) contains an array of heterogeneous block types, use the `Extract` and `NonNullable` utilities to isolate specific components:

```typescript
// Example: Extracting a specific block type from a Sanity PageBuilder array
type FeaturesBlockData = Extract<
  NonNullable<NonNullable<PAGE_QUERY_RESULT>["pageBuilder"]>[number],
  { _type: "featuresBlock" }
>;

// The component is now 100% type-safe without using `any`
export function FeaturesBlock({ data }: { data: FeaturesBlockData }) { ... }
```

### Typing Map Callbacks
Instead of allowing `(block: any)` in an array map, extract the union type for the entire array:

```typescript
type BlockType = NonNullable<NonNullable<PAGE_QUERY_RESULT>["pageBuilder"]>[number];

// Applied cleanly to the array iteration
{blocks.map((block: BlockType, index: number) => { ... })}
```

### Sanity TypeGen Naming Conventions
When using `@sanity/typegen` or Next-Sanity type extraction, the generated type names strictly follow the casing of the query constant. Do not guess the suffix.
- If your query constant is `const PAGE_QUERY = ...`, the generated type is `PAGE_QUERY_RESULT`.
- If your query constant is `const pageQuery = ...`, the generated type is `pageQueryResult`.
Always double-check `sanity.types.ts` for the exact generated name to avoid `has no exported member` TypeScript errors.

### Schema Extraction Requirement
If you update your schema (e.g. adding a new field), `sanity typegen generate` will often **fail to pick up the new fields** unless the static schema file is updated first. Always run:
```bash
npx sanity schema extract
npx sanity typegen generate
```
This ensures the `schema.json` is updated before the TypeScript types are built.

### Typing Untyped Plugin Fields (e.g. Colors)
Some plugins like `@sanity/color-input` generate `unknown` or omit the field type entirely in `sanity.types.ts`. Do NOT cast these to `any`. Instead, define an explicit type interface and cast `unknown` to it cleanly:
```typescript
type SanityColor = { hex?: string };
// Safely cast the specific field instead of the whole object
const bgColor = (data.backgroundColor as SanityColor | undefined)?.hex;
```

### Deeply Nested Array Extraction
For heavily nested optional arrays (like menu links in a header reference), use `NonNullable` repeatedly to safely drill down to the array element type:
```typescript
type HeaderData = NonNullable<PAGE_QUERY_RESULT>["header"];
type MenuLink = NonNullable<NonNullable<HeaderData>["menuLinks"]>[number];

// Usage in map
{data.menuLinks?.map((link: MenuLink) => ( ... ))}
```
